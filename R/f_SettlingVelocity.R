...
#' @export
f_SettlingVelocity <- function(rad_species, rho_species, matrix.Rho, DynVisc,
                             Matrix) {
  ...
  switch (Matrix,
          "water" = {
            ...
            2*(rad_species^2*(rho_species-matrix.Rho)*getConst("gn")) / (9*DynVisc) #,
            #        SettlVelocitywater # what is this? Does not make sense.
            #      )
          },
          "air" = {
            Cunningham <- f_Cunningham(rad_species)
            2*(rad_species^2 * (rho_species - matrix.Rho)*getConst("gn")*Cunningham)/(9*DynVisc) # particle settling velocity
          },
          NA #else
  )
}
