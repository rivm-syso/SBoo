  ...
  R6::R6Class(
    ...
    public = list(
        ...
        if(ParallelPreparation == T){
          ...
          if (nRUNs < 2){
            ...
            if (nVars > 0){
              ...
              new_names <- apply(as.data.frame(do.call(rbind, strsplit(rnames, ",q9_k"))), 1, paste, collapse = " ")
          ...
          if(is.null(LHSmatrix)){
            ...
            if (nVars > 0) { # MIND YOU: the private lhsruns are for variables only!
              ...
              private$LHSruns <- t(scaled_samples)
              rownames(private$LHSruns) <- gsub(",q9_k", " ", rownames(private$LHSruns))
      ...
      PrepLHS = function(var_box_df = data.frame(), var_invFun = list(), emis_invFun = list(), nRUNs = 100) {
        ...
        if(!is.null(private$input_variables)){
          colnames_lhs <- paste0(private$input_variables$varName, ",q9_k", private$input_variables$Scale, ",q9_k", private$input_variables$SubCompart, ",q9_k", private$input_variables$Species)
        } else{
          colnames_lhs <- c()
        }
      ...
      TransformCorrelatedLHS = function(lhsRUNs, correlations, var_invfun) {
        ...
        correlations <- data.frame(varName_1 = paste0(correlations$varName_1, ",q9_k", correlations$Scale_1, ",q9_k", correlations$SubCompart_1, ",q9_k", correlations$Species_1),
                                   varName_2 = paste0(correlations$varName_2, ",q9_k", correlations$Scale_2, ",q9_k", correlations$SubCompart_2, ",q9_k", correlations$Species_2),
      ...
      ExpandLHS = function(lhsRUNs){
        ...
        for(i in seq_len(ncol(lhsRUNs))) {
          ...
          name_parts <- strsplit(cname, ",q9_k")[[1]]
          ...
          if (is.null(needed_states)) {
            ...
            if ("Species" %in% needed_states && (current_species == "any" || current_species == "NA")) {
              for (species_type in species) {
                col_name <- paste(varname, current_scale, current_subcompart, species_type, sep = ",q9_k")
            ...
            for (expanded_col_name in names(expanded_columns_species)) {
              ...
              parts <- strsplit(expanded_col_name, ",q9_k")[[1]]
              scale <- parts[2]
              
              if ("Scale" %in% needed_states && (scale == "any" || scale == "NA")) {
                for (scale_type in scales) {
                  col_name <- paste(parts[1], scale_type, parts[3], parts[4], sep = ",q9_k")
        ...
        vars <- sapply(strsplit(cnames, ",q9_k"), `[`, 1)
        scales <- sapply(strsplit(cnames, ",q9_k"), `[`, 2)
        subcomparts <- sapply(strsplit(cnames, ",q9_k"), `[`, 3)
        species <- sapply(strsplit(cnames, ",q9_k"), `[`, 4)
