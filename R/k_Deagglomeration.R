...
k_Deagglomeration <- function (kdeag, SubCompartName, ScaleName, Test_surface_water){
  if (((ScaleName %in% c("Tropic", "Moderate", "Arctic")) & (SubCompartName == "freshwatersediment" | 
    ...
    return(NA)
  } else if (!is.null(ScaleName) && ScaleName %in% c("Regional", "Continental") && (SubCompartName == "deepocean") && (is.na(Test_surface_water) || isFALSE(Test_surface_water) || Test_surface_water == "FALSE")){
    return(NA)
  } else {
    return(kdeag)
  }
