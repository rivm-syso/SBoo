...
#' @export
SettlingVelocity <- function(rad_species, rho_species, rhoMatrix, DynVisc,
                             Matrix) {
  stopifnot(is.numeric(rho_species), is.numeric(rhoMatrix), is.numeric(DynVisc))
  ...
  switch (Matrix,
          "water" = {
            ...
            2*(rad_species^2*(rho_species-rhoMatrix)*GN) / (9*DynVisc) #,
            #        SettlVelocitywater # what is this? Does not make sense.
            #      )
          },
          "air" = {
            Cunningham <- f_Cunningham(rad_species)
            2*(rad_species^2 * (rho_species - rhoMatrix)*GN*Cunningham)/(9*DynVisc) # particle settling velocity
          },
          NA #else
  )
}
